...
void record_preferences(int ranks[])
{
    ...
    for (int i = 0; i < candidate_count; i++){
        ...
        for (int j = 0; j < candidate_count; j++){
            ...
            // If the first candidate ranks higher than the second
            if (ranks[i] < ranks[j]){
                preferences[i][j]++;
            }
        }
...
void sort_pairs(void)
{
    ...
    while (change > 0){
        ...
        for (int i = 0; i < pair_count - 1; i++){
            // If the pair is weaker than the next pair
...
void lock_pairs(void)
{
    // For every pair starting at the strongest
...
bool check_for_cycle (int winner, int loser){
    ...
    // For every candidate
    for (int i = 0; i < candidate_count; i++){
        // If the loser is locked in over the candidate
        if (locked[winner][i]){
            // If the candidate is locked in over the winner
            if (locked[i][loser]){
                    return true;
            }
            // If the candidate is not locked in over the winner
            else{
                if (check_for_cycle(i, loser)){
                    return true;
                }
            }
        }
